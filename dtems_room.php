...
$classroom = isset($_POST['classroom']) ? $_POST['classroom'] : 'TRA-0R-01'; // Default classroom
$days = "";
$startTime = "";
$endTime = "";
$status = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" || !$classroom) {
    ...
    $sql = "SELECT room_id, room_name, start_time, end_time, status, days, user_id FROM dtems_rooms WHERE room_name = ?";
...
<head>
    ...
    <title>DTEMS Room Booking</title>
    <style>
        html, body {
            height: 100%;
            margin: 0;
    ...
    <div class="locations">
        <form id="classroomForm" method="post" action="">
            ...
            <select id="classroom" name="classroom">
                <option value="TRA-0R-01" <?php if ($classroom == 'TRA-0R-01') echo 'selected'; ?>>TRA-0R-01</option>
                <option value="TRA-0R-02" <?php if ($classroom == 'TRA -0R-02') echo 'selected'; ?>>TRA-0R-02</option>
                <option value="TRA-0R-03" <?php if ($classroom == 'TRA-0R-03') echo 'selected'; ?>>TRA-0R-03</option>
...
<script>
        ...
        xhr.onreadystatechange = function() {
            if (xhr.readyState == 4) {
                try {
                    var response = JSON.parse(xhr.responseText);
                    if (response.success) {
                        // Hide cancel modal and show success popup
    ...
    function lecturerOverride(room, day, time, studentId) {
        ...
        if (confirm('Are you sure you want to override this student booking? An email will be sent to notify them.')) {
            ...
            const formData = `room=${encodeURIComponent(room)}&day=${encodeURIComponent(day)}&time=${encodeURIComponent(time)}&student_id=${encodeURIComponent(studentId)}&room_type=tra`;
