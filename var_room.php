...
$classroom = isset($_POST['classroom']) ? $_POST['classroom'] : 'VAR-0R-01'; // Default classroom
$days = "";
$startTime = "";
$endTime = "";
$status = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" || !$classroom) {
    ...
    $sql = "SELECT room_id, room_name, start_time, end_time, status, days, user_id FROM var_rooms WHERE room_name = ?";
    ...
    <div class="locations">
        <form id="classroomForm" method="post" action="">
            ...
            <select id="classroom" name="classroom">
                <option value="VAR-0R-01" <?php if ($classroom == 'VAR-0R-01') echo 'selected'; ?>>VAR-0R-01</option>
                <option value="VAR-0R-02" <?php if ($classroom == 'VAR-0R-02') echo 'selected'; ?>>VAR-0R-02</option>
                <option value="VAR-0R-03" <?php if ($classroom == 'VAR-0R-03') echo 'selected'; ?>>VAR-0R-03</option>
...
<script>
    ...
    function lecturerOverride(room, day, time, studentId) {
        ...
        if (confirm('Are you sure you want to override this student booking? An email will be sent to notify them.')) {
            ...
            const formData = `room=${encodeURIComponent(room)}&day=${encodeURIComponent(day)}&time=${encodeURIComponent(time)}&student_id=${encodeURIComponent(studentId)}&room_type=var`;
